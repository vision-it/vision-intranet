      ...
      'intranet' => {
        ...
        'deploy' => {
          'labels' => [
            ...
            'traefik.http.routers.intranet.middlewares=strip-intranet@docker,fhg-intranet@docker',
            'traefik.http.middlewares.strip-intranet.stripprefix.prefixes=/intranet',
            'traefik.http.middlewares.fhg-intranet.redirectregex.regex=^https?://(.+).fhg.de/(.*)',
            'traefik.http.middlewares.fhg-intranet.redirectregex.replacement=https://$${1}.fraunhofer.de/$${2}',
            'traefik.http.middlewares.fhg-intranet.redirectregex.permanent=true',
          ],
        },
      },
      'intranet-queue' => {
